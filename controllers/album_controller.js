...
const { User, Album, Photo, Albums_Photos } = require('../models')
...
const createAlbum = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send({
      status: 'error',
      message: 'Exception thrown in database when creating a new album, (err1)',
    })
    throw errors
  }
}
...
const storePhotos = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send({
      status: 'error',
      message: 'error when trying to add photo to album (err2)',
    })
    throw error
  }
}
